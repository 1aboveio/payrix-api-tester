...
stateDiagram-v2
    ...
    支付成功 --> 已撤销: POST /void (当日)
    支付成功 --> 已冲正: POST /reversal (当日)
    支付成功 --> 已退款: POST /refund
    支付成功 --> 原路退回: POST /return
...
| 支付成功 | `POST /reversal`      | 已冲正     | 当日全额撤销       |
| 支付成功 | `POST /refund`        | 已退款     | 独立退款（客户在场）   |
| 支付成功 | `POST /return`        | 原路退回    | 原路退回（无需客户在场） |
| 已授权  | `POST /completion`    | 已完成     | 完成预授权，扣款     |
| 已授权  | `POST /void`          | 已撤销     | 取消预授权，解冻资金   |
| 已授权  | `POST /reversal`      | 已冲正     | 冲正预授权        |
