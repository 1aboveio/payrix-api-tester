...
export default function AuthorizationPage() {
  ...
  const [form, setForm] = useState<AuthorizationRequest>({ ...DEFAULTS, laneId: activeTripos(config).defaultLaneId || '', referenceNumber: generateReferenceNumber(), ticketNumber: generateTicketNumber() });
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <TemplateSelector
            templates={authorizationTemplates}
            selectedId={templateId}
            onSelect={(tpl) => {
              setTemplateId(tpl.id);
              setTemplateName(tpl.name);
              setForm({ ...DEFAULTS, laneId: activeTripos(config).defaultLaneId || '', ...tpl.fields } as AuthorizationRequest);
            }}
            onReset={() => {
              setTemplateId('');
              setTemplateName('');
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <Input id="amount" value={form.transactionAmount} onChange={(e) => setForm({ ...form, transactionAmount: e.target.value })} placeholder="10.00" required />
            </div>
            <div className="space-y-2">
          ...
          transactionId ? (
            <>
              <Button asChild variant="outline">
                <Link href={`/transactions/completion?transactionId=${encodeURIComponent(transactionId)}`}>Completion</Link>
              </Button>
              <Button asChild variant="outline">
                <Link href={`/reversals/void?transactionId=${encodeURIComponent(transactionId)}`}>Void</Link>
              </Button>
              <Button asChild variant="outline">
                <Link href={`/reversals/reversal?transactionId=${encodeURIComponent(transactionId)}&paymentType=credit`}>Reversal</Link>
              </Button>
            </>
          ) : null
        }
