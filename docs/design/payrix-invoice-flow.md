...
客户打开该链接即可在 Payrix hosted 页面直接支付。

---

## 支付方式：信用卡 + ACH Direct Debit

Invoice 通过 `allowedPaymentMethods` 字段控制客户可使用的支付方式。支持信用卡和 ACH（银行账户直接扣款）两大类。
...
# 信用卡 + ACH
"allowedPaymentMethods": "visa|masterCard|amex|discover|checking|savings"

# 仅 ACH
"allowedPaymentMethods": "checking|savings|corpChecking|corpSavings"
```

> 多个值用 `|` 分隔。Payrix hosted 支付页面会根据此字段自动展示对应的支付选项。

### ACH 付款的交易类型

当客户通过 ACH 付款时，系统创建的是 eCheck 交易（而非信用卡交易）：

| txn type | 说明 |
|----------|------|
| `7` | eCheck Sale（ACH 扣款） |
| `8` | eCheck Refund（ACH 退款） |
| `11` | eCheck Redeposit（失败后重新提交） |
| `12` | eCheck Account Verification（账户验证） |

### ACH Token 存储

如需保存客户银行账户信息用于后续自动扣款，Token 的 `payment.method` 使用不同的值：
...
> ACH token 需要额外的 `routing`（银行路由号）字段，信用卡 token 不需要。
