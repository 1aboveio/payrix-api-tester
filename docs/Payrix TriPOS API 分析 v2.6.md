...
**交易控制参数：**
- `invokeManualEntry` (boolean) - 直接调用手动输入模式（用于 Keyed 测试）
- `requestedCashbackAmount` (number) - 请求的 Cashback 金额（仅限 Debit 卡）
- `tipAmount` (number) - 小费金额（餐厅场景）
- `salesTaxAmount` (number) - 销售税金额（Level II 处理）
- `commercialCardCustomerCode` (string) - 商业卡客户代码（Level II，如 PO Number）

**EBT 相关：**
- `ebtType` (string) - EBT 卡类型："FoodStamp" 或 "CashBenefit"
- `foodStampAmount` (number) - 食品券金额

**Store & Forward：**
- Query参数 `action` (string) - "store" 或 "forward"（存储转发功能）
- Query参数 `requestIdToForward` (string) - 要转发的存储交易请求ID

**Configuration 对象（覆盖 triPOS.config 设置）：**
- `configuration.allowDebit` (boolean) - 允许 Debit 卡
- `configuration.allowPartialApprovals` (boolean) - 允许部分批准
- `configuration.checkForDuplicateTransactions` (boolean) - 启用重复交易检查
- `configuration.marketCode` (string) - 市场代码（Retail, HotelLodging, FoodRestaurant等）
- `configuration.cashbackOptions` (object) - Cashback 选项配置

完整参数列表参见 [Sale API 规范](https://docs.worldpay.com/apis/tripos/tripos-cloud/tripos-transaction/api-specification#tag/Sale/operation/postv1sale)
...
**交易控制参数：**
- `invokeManualEntry` (boolean) - 直接调用手动输入模式（用于 Keyed 测试）
- `tipAmount` (number) - 小费金额（餐厅预授权场景）
- `salesTaxAmount` (number) - 销售税金额（Level II 处理）
- `commercialCardCustomerCode` (string) - 商业卡客户代码（Level II）

**Store & Forward：**
- Query参数 `action` (string) - "store" 或 "forward"
- Query参数 `requestIdToForward` (string) - 要转发的存储交易请求ID

**Configuration 对象：**
- `configuration.allowPartialApprovals` (boolean) - 允许部分批准
- `configuration.marketCode` (string) - 市场代码（HotelLodging 等）

完整参数列表参见 [Authorization API 规范](https://docs.worldpay.com/apis/tripos/tripos-cloud/tripos-transaction/api-specification#tag/Authorization/operation/postv1authorization)
...
## 5. POST /api/v1/refund/{paymentAccountId} — Refund（独立退款，需终端交互）
> **API 文档：**  
...
> - OpenAPI 规范：https://docs.worldpay.com/assets/api-specs/tripos-transaction/api-specification/Tripos%20Transaction%20API-4.8.0.yaml  
> - 端点：`POST /api/v1/refund/{paymentAccountId}`
...
**必需参数（Required）：**
- `laneId` (integer) - Lane ID
- `transactionAmount` (string) - 交易金额
- **`approvalNumber` (string, REQUIRED)** - 语音授权获得的批准码（来自电话授权）
...
**可选参数（认证测试可能需要）：**
- `invokeManualEntry` (boolean) - 手动输入模式（用于 Keyed Force 测试）
- `salesTaxAmount` (number) - 销售税金额（Level II 处理）
- `commercialCardCustomerCode` (string) - 商业卡客户代码（Level II）

**Configuration 对象：**
- `configuration.marketCode` (string) - 市场代码

完整参数列表参见 [Force API 规范](https://docs.worldpay.com/apis/tripos/tripos-cloud/tripos-transaction/api-specification#tag/Force/operation/postv1forcecredit)
...
**必需参数：**
- **路径参数：** `laneId` (integer, required)

**可选参数（认证测试需要）：**
- **Query 参数：** `invokeManualEntry` (boolean) - 直接调用手动输入模式
- **Query 参数：** `isCscSupported` (string) - 提示持卡人输入 CSC（卡安全码）
...
# BQ-3: BIN Query with Manual Entry (for keyed card entry test)
